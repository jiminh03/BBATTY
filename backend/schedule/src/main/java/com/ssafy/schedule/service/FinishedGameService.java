...
public class FinishedGameService {
    ...
    public int crawlAndUpdateFinishedGames(String date) {
        log.info("{} 야구 경기 결과 업데이트 크롤링 시작", date);
        ...
        for (String gameUrl : gameUrls) {
            if (crawlAndUpdateFinishedGameDetail(gameUrl, date)) {
                updatedCount++;
            }
        }
    ...
    public int crawlAndSaveFinishedGames(String date) {
        log.info("{} 야구 경기 결과 직접 저장 크롤링 시작", date);
        ...
        int savedCount = 0;
        for (String gameUrl : gameUrls) {
            if (crawlAndSaveFinishedGameDetail(gameUrl, date)) {
                savedCount++;
            }
        }
        
        log.info("{}개의 경기 결과 직접 저장 완료", savedCount);
        return savedCount;
    }

    @Transactional
    public boolean crawlAndUpdateFinishedGameDetail(String gameUrl, String dateStr) {
        log.info("경기 결과 업데이트 크롤링: {}", gameUrl);
        ...
        if (scoreElements.size() < 2) {
            log.info("점수가 없는 경기 - 결과 업데이트 건너뛰기: {} vs {}", awayTeamName, homeTeamName);
            return false;
        }
        ...
        try {
            awayScore = Integer.parseInt(scoreElements.get(0).ownText());
            homeScore = Integer.parseInt(scoreElements.get(1).ownText());
            
            log.info("진행된 경기 발견: {} vs {} ({}:{})", awayTeamName, homeTeamName, awayScore, homeScore);
        ...
        if (homeTeamName != null && awayTeamName != null && gameTime != null && 
            ...
            return updateFinishedGame(homeTeamName, awayTeamName, gameTime, dateStr, homeScore, awayScore);
        } else {
            log.warn("필수 정보 누락 - 홈팀: {}, 원정팀: {}, 시간: {}, 점수: {}:{}", 
                    homeTeamName, awayTeamName, gameTime, homeScore, awayScore);
            return false;
        }
    }

    @Transactional
    public boolean crawlAndSaveFinishedGameDetail(String gameUrl, String dateStr) {
        log.info("경기 결과 직접 저장 크롤링: {}", gameUrl);
        ...
        if (scoreElements.size() < 2) {
            log.info("점수가 없는 경기 - 결과 저장 건너뛰기: {} vs {}", awayTeamName, homeTeamName);
            return false;
        }
        ...
        if (homeTeamName != null && awayTeamName != null && gameTime != null && 
            awayScore != null && homeScore != null) {
            return saveFinishedGame(homeTeamName, awayTeamName, gameTime, dateStr, homeScore, awayScore);
        } else {
            log.warn("필수 정보 누락 - 홈팀: {}, 원정팀: {}, 시간: {}, 점수: {}:{}", 
                    homeTeamName, awayTeamName, gameTime, homeScore, awayScore);
            return false;
    ...
    private boolean updateFinishedGame(String homeTeamName, String awayTeamName, String gameTime, 
        ...
        try {
            ...
            if (existingGame == null) {
                log.warn("기존 경기 일정을 찾을 수 없음: {} vs {} on {}", 
                        mappedHomeTeamName, mappedAwayTeamName, dateStr);
                return false;
            }

            GameResult gameResult = calculateGameResult(homeScore, awayScore);
    ...
    private boolean saveFinishedGame(String homeTeamName, String awayTeamName, String gameTime, 
        ...
        try {
            ...
            if (gameRepository.existsByHomeTeamAndAwayTeamAndDate(homeTeam, awayTeam, gameDateTime)) {
                log.info("이미 존재하는 경기 - 건너뛰기: {} vs {} on {}", 
                        mappedHomeTeamName, mappedAwayTeamName, dateStr);
                return false;
            }
            ...
            log.info("✅ 경기 결과 직접 저장 완료: {} vs {} ({}:{}) - 결과: {}", 
                    mappedAwayTeamName, mappedHomeTeamName, awayScore, homeScore, gameResult);
            return true;

        } catch (Exception e) {
            log.error("경기 결과 직접 저장 실패: {}", e.getMessage(), e);
            return false;
        }
    }

    private GameResult calculateGameResult(Integer homeScore, Integer awayScore) {
