...
export const useMessageQueue = ({ onSendMessage, isConnected, maxRetries = 3 }: UseMessageQueueProps) => {
  ...
  const updateMessageStatus = useCallback((messageId: string, status: MessageStatus) => {
    setPendingMessages((prev) => prev.map((msg) => (msg.id === messageId ? { ...msg, status } : msg)));
  }, []);

  // 큐에서 메시지 업데이트
  const updateMessageInQueue = useCallback((messageId: string, updatedMessage: PendingMessage) => {
    setPendingMessages((prev) => prev.map((msg) => (msg.id === messageId ? updatedMessage : msg)));
  }, []);
  ...
  useEffect(() => {
    if (isConnected) {
      const waitingMessages = pendingMessages.filter((msg) => msg.status === 'sending' || msg.status === 'failed');
