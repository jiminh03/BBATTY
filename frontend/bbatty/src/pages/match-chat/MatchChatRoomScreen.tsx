...
} from '../../features/match-chat/components/ConnectionStatus';
...
export const MatchChatRoomScreen = () => {
  ...
  const handleProfilePress = useCallback(
    (userId: string) => {
      ...
      if (!isNaN(numericUserId)) {
        // 다른 사람의 프로필은 OtherProfile 화면으로 이동
        // @ts-ignore - 네비게이션 타입 무시 (MyPageStack으로 이동)
        navigation.navigate('MainTabs', {
          screen: 'MyPageStack',
          params: {
            screen: 'OtherProfile',
            params: { userId: numericUserId, fromChatRoom: true },
          },
        });
      }
    },
  ...
  // 워치채팅 여부 확인
  const isWatchChat =
    websocketUrl.includes('/ws/watch-chat/') || (websocketUrl.includes('gameId=') && websocketUrl.includes('teamId='));
  ...
  const { pendingMessages, addMessageToQueue, retryMessage, removeMessage, flushQueue } = useMessageQueue({
  ...
  const addMessage = useCallback((message: ChatMessage, isMyMessage: boolean = false) => {
    setMessages((prev) => {
      const isDuplicate = prev.some(
        (m) => m.timestamp === message.timestamp && m.content === message.content && m.userId === message.userId
      );
    ...
    try {
      ...
      websocket.onmessage = (event) => {
        ...
        try {
                ...
                if (isMyMessage) {
                  setMessages((prev) => {
                    ...
                    const filtered = prev.filter(
                      (m) => !(m.content === messageData.content && (m as any)._isMyMessage && m.status)
                    );
          ...
          } else if (messageData.messageType === 'USER_JOIN' || messageData.messageType === 'USER_LEAVE') {
  ...
  return (
    <View style={styles.container}>
      ...
      <ChatNotificationManager notifications={notifications} onDismiss={dismissNotification} />
          ...
          ListFooterComponent={
            isLoadingMore ? (
              <View style={{ paddingVertical: 20, alignItems: 'center' }}>
                ...
                <Text style={{ marginTop: 8, color: '#666', fontSize: 12 }}>이전 메시지를 불러오는 중...</Text>
              </View>
            ) : null
          }
          renderItem={({ item }) => (
                ...
                >
                  ...
                  {!((item as any)._isMyMessage || (item as any)._isPending) && (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        ...
                        if (message.userId) {
                          handleProfilePress(message.userId.toString());
                        }
                      }}
                    >
                      {(() => {
                  ...
                  <View
                    style={[
                      ...
                      (item as any)._isMyMessage || (item as any)._isPending
                        ? styles.myMessageBubbleContainer
                        : styles.otherMessageBubbleContainer,
                    ]}
                  >
                    {/* 닉네임 (내 메시지가 아닌 경우만) */}
                    {!((item as any)._isMyMessage || (item as any)._isPending) && (
                      <Text style={styles.messageNickname}>{(item as MatchChatMessage).nickname}</Text>
                    )}
                    ...
                    <View
                      style={[
                        ...
                        (item as any)._isMyMessage || (item as any)._isPending
                          ? styles.myBubbleAndTimeContainer
                          : styles.otherBubbleAndTimeContainer,
                      ]}
                    >
                      {/* 시간 (내 메시지인 경우 왼쪽에) */}
                      {((item as any)._isMyMessage || (item as any)._isPending) && (
