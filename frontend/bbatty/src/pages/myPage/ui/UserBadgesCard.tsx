...
export const UserBadgesCard: React.FC<UserBadgesCardProps> = ({ badgeCategories }) => {
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      {/* 구장정복 카테고리 */}
      {badgeCategories
        .filter((c) => c.category === 'STADIUM_CONQUEST')
        .map((category) => (
          <View key={category.category}>
            <View style={[styles.categoryCard, { backgroundColor: getCategoryColor(category) + '10' }]}>
              <Text style={[styles.categoryTitle, { color: getCategoryColor(category) }]}>방문구장</Text>
            ...
            return (
              <View key={category.category} style={[styles.categorySection, { backgroundColor: categoryColor + '10' }]}>
                <Text style={[styles.categoryTitle, { color: categoryColor }]}>
                  {category.category === 'SEASON_GAMES' ? '경기' : category.displayName}
                </Text>
                <View style={styles.categoryBadges}>
                  {category.badges.map((badge, index) => (
                    <View key={badge.badgeType} style={isWinsCategory ? styles.winsBadgeItem : styles.smallBadgeItem}>
                      <View
                        style={[
                          styles.smallBadgeIcon,
                          badge.acquired ? { backgroundColor: categoryColor } : { backgroundColor: '#E0E0E0' },
                        ]}
                      >
                        {badge.acquired ? (
